---
category: Siege weapon
ac: 19 
hp: 20
source: XDMG
aliases:
- Suspended Cauldron
image: 2. Compendium/objects/token/suspended-cauldron-xdmg.webp
tags: Object
---
# Suspended Cauldron
*Source: Dungeon Master's Guide (2024) p. 97* 

An iron pot is suspended so that it can be tipped easily, spilling its contents. Once emptied, a cauldron must be refilled before it can be used again. Filling the cauldron requires three [Utilize](0.%20DM%20Screen/actions.md#Utilize) actions. Then a crew member can take the Spill action.

Cauldrons are typically filled with boiling oil but can be filled with other substances, such as acid or [green slime](green-slime-xdmg.md) (see ""Hazards""), with different effects.
```ad-statblock
title: Suspended Cauldron

*Large Siege weapon*
![[objects/token/suspended-cauldron-xdmg.webp#right|200]]
- **Armor Class** 19 
- **Hit Points** 20 
- **Speed** 

|STR|DEX|CON|INT|WIS|CHA|
|:---:|:---:|:---:|:---:|:---:|:---:|
|10 (+0)|10 (+0)|10 (+0)|10 (+0)|10 (+0)|10 (+0)|


## Actions

***Spill (Requires a Full Cauldron).*** *Dexterity Saving Throw:* DC 15, each creature in a 10-foot square directly below the cauldron. *Failure:* 10 (`3d6`) Fire damage. *Success:* Half damage.
```
^statblock